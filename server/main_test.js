const Promise = require('bluebird')
const AppDataAccessObject = require('./app_data_access_object')
const ProjectRepository = require('./project_repository')
const TaskRepository = require('./task_repository')

function main() {
  const dao = new AppDataAccessObject('./database.sqlite3')
  const blogProjectData = { name: 'My E-PortFolio' }
  const projectRepo = new ProjectRepository(dao)
  const taskRepo = new TaskRepository(dao)
  let projectId

  projectRepo.createTable()
    .then(() => taskRepo.createTable())
    .then(() => projectRepo.create(blogProjectData.name))
    .then((data) => {
      projectId = data.id
      const tasks = [
        {
          name: 'Front',
          description: 'Le visuel en react',
          isComplete: 0,
          projectId
        },
        {
          name: 'DB',
          description: 'La base de données en SQLite',
          isComplete: 1,
          projectId
        },
        {
            name: 'API',
            description: 'API permettant la communication back front',
            isComplete: 0,
            projectId
        },
        {
            name: 'Connection',
            description: 'Connection me permettant à moi seul dajouter des projets',
            isComplete: 0,
            projectId
        },
        {
            name: 'Serveur Nodejs',
            description: 'Le serveur en Node JS',
            isComplete: 1,
            projectId
        }
      ]
      return Promise.all(tasks.map((task) => {
        const { name, description, isComplete, projectId } = task
        return taskRepo.create(name, description, isComplete, projectId)
      }))
    })
    .then(() => projectRepo.getById(projectId))
    .then((project) => {
      console.log(`\nRetreived project from database`)
      console.log(`project id = ${project.id}`)
      console.log(`project name = ${project.name}`)
      return projectRepo.getTasks(project.id)
    })
    .then((tasks) => {
      console.log('\nRetrieved project tasks from database')
      return new Promise((resolve, reject) => {
        tasks.forEach((task) => {
          console.log(`task id = ${task.id}`)
          console.log(`task name = ${task.name}`)
          console.log(`task description = ${task.description}`)
          console.log(`task isComplete = ${task.isComplete}`)
          console.log(`task projectId = ${task.projectId}`)
        })
      })
      resolve('success')
    })
    .catch((err) => {
      console.log('Error: ')
      console.log(JSON.stringify(err))
    })
}

main()